...
//        Uncomment the following code snippet to check whether any of the field is empty.
        ...
        String filterButton = req.getParameter("filter");
        if(filterButton!=null){
            String tagSelected = req.getParameter("tag");
            try {
                req.setAttribute("dao",((PostDAO)DAOFactory.postsCRUD()).findByTag(tagSelected));
                RequestDispatcher rd = req.getRequestDispatcher("/blog/Filter.jsp"); //rd is RequestDispatcher
                rd.forward(req, resp);
                return;
        ...
        PostDAO dao = (PostDAO) DAOFactory.postsCRUD();
        try {
            dao.create(dto);
        } catch (SQLException throwables) {
            throwables.printStackTrace();
...
//        out.println(dto.getDescription());
//        out.println(dto.getPostId());
//        out.println(dto.getTag());
//        out.println(dto.getTimestamp());
//        out.println(dto.getTitle());

        req.setAttribute("dtoObj", dto);
        RequestDispatcher rd = req.getRequestDispatcher("/blog/ViewPostById.jsp"); //rd is RequestDispatcher
        rd.forward(req, resp);
