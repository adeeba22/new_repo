...
import com.upgrad.blog.dao.UserDAO;
...
public class UserServlet extends HttpServlet {
    ...
    protected void doPost(HttpServletRequest req, HttpServletResponse resp) throws ServletException, IOException {
        ...
        String signIn = req.getParameter("actionTypeSignIn");
        String signUp = req.getParameter("actionTypeSignOut");
        ...
        if (null != email && null!=signIn) {
            try {

                emailValidator.isValidEmail(email);
                UserDAO userDAO = new UserDAO();
               UserDTO dto = userDAO.findByEmail(email);
               if(null==dto || null==dto.getEmailId()){
                   errors.add("No user registered with the given email address!");
               }else{
                   if(!password.equals(dto.getPassword())){
                       errors.add("Please enter valid credentials");
                   }
               }
        ...
        if (null != signIn) {
            ...
            req.setAttribute("emailId", strings[0]);
            out.println("User Signed In : " + strings[0]);
            System.out.print("User Signed In : " + strings[0]);
